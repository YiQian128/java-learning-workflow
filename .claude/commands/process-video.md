...
处理单个 Java 教程视频（流程 A），生成视频级知识文档（`knowledge_{stem}.md`）。
...
- `--force` — 强制重新生成知识文档，覆盖已有 `knowledge_{stem}.md`
...
```
portable-gpu-worker/output/{课程文件夹}/{章节文件夹}/{video_stem}/
├── knowledge_{stem}.md          ← 唯一产物（Full/Supplement/DeepDive/Practice 之一）
└── _preprocessing/              ← 预处理产物
    ├── *.srt
    ├── *_words.json
    ├── *_topics.json            ← A1 输出（含处理模式）
    ├── *_teaching_style.json    ← A1 输出
    └── frames/
...
```
check_preprocessing_status(video_path)    → 预处理产物状态
get_video_metadata(video_path)            → 视频时长/帧率等信息
get_output_paths(video_name)              → 标准化输出路径
query_knowledge_graph(concept_ids=[...])  → 查询已覆盖概念状态（章节过滤）
```

**知识图谱查询**：根据视频文件名/字幕内容推断本视频可能涉及的概念，查询其在图谱中的覆盖状态，用于 Step 3 的模式判断。
...
输出：`_topics.json`、`_teaching_style.json`
...
输出：`knowledge_{stem}.md`

### Step 5：保存产物

将 `knowledge_{stem}.md` 保存到标准路径：
```
portable-gpu-worker/output/{课程文件夹}/{章节文件夹}/{video_stem}/
```
...
📄 知识文档：output/{课程}/{章节}/{stem}/knowledge_{stem}.md
