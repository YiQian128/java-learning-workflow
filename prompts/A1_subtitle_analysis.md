...
三项核心任务：
1. **字幕噪声处理 + 话题分段**：从字幕中剔除噪声（见下方"字幕预压缩"），提取结构化话题清单 → `{safe_stem}_topics.json`（segments 部分）
2. **知识图谱比对 + 处理模式判断**：基于 `KNOWLEDGE_GRAPH_DATA` 分析概念重叠情况，为本视频分配处理模式（Full/Supplement/DeepDive/Practice）→ `{safe_stem}_topics.json`（video_info.processing_mode 等字段）
3. **教学风格提取**：分析讲师讲解方式（类比、节奏、难点标记），为 A2 阶段提升知识文档质量 → `{safe_stem}_teaching_style.json`

**以上三项任务的输出均保存到 `{PREPROCESSING_DIR}` 目录下。**
...
收到 `SUBTITLE_TEXT` 后，**在执行任何分析步骤之前**，先对字幕做以下三步压缩，压缩结果作为后续所有步骤的 `SUBTITLE_TEXT`：
...
> 📎 四种模式的触发条件摘要、文档特征和局部切换规则，见 SKILL.md §流程A → Stage 1 内部决策。此处给出精确计算算法：
...
模式判断：
  ...
  ELIF new_concept_ratio < 0.20 AND max_depth_delta ≤ 0:
    → Practice Mode（实操模式：极简步骤文档，≤1000字）
...
```json
{
  "video_info": {
    ...
    "directory": "<相对于 portable-gpu-worker/videos 的目录，例如 Java基础-视频上/day04-判断和循环>",
    "duration": "{DURATION}",
    "core_topic": "本视频核心主题",
    ...
    "output_path": "<相对于 portable-gpu-worker/output 的路径，例如 Java基础-视频上/day04-判断和循环/01-示例/_preprocessing/01-示例_topics.json>",
    "processing_mode": "Full",
...
```json
{
  ...
  "output_path": "<相对于 portable-gpu-worker/output 的路径，例如 Java基础-视频上/day04-判断和循环/01-示例/_preprocessing/01-示例_teaching_style.json>",
  "analogies": [...],
  "pace_analysis": [
    // Step 6.2 的数组内容
  ],
