...
| 来源 | 地址 | 用途 |
|------|------|------|
| JavaGuide（153k?） | https://javaguide.cn | 面试题/概念解释首选参考 |
...
> **? 核心执行规范。处理每个视频必须严格按流程 A 的步骤顺序执行，不可跳过任何步骤。**
...
| **Practice** | 新话题 < 20%，且所有概念 Δdepth=0（纯操作练习） | 极简（只写步骤/命令/代码）；练习题数量较其他模式多 50% |
...
> ? 跳过此步将导致下一视频无法正确判断处理模式，可能重复生成已有内容或遗漏深化。
...
**触发条件**（任意满足即可）：
- 用户显式要求（如"帮我整合 day01 的内容"）
- 系统检测到当前章节目录下所有视频都已有对应知识文档
- 用户开始处理下一章节时，系统询问"是否先生成上一章节综合？"

**Step C1 — 扫描章节状态**

> ? **兜底机制 2 — 知识完整性核查（本步执行，不可省略）**
...
调用 scan_chapter_completeness(chapter_dir)
  → 输出"待补全清单"，示例：
    ? java.jdk_jre_jvm：internals 面 → 待 JVM 原理章节覆盖
    ? java.public_static_void：depth=0.5（代码中出现但未解释）→ 待 OOP 章节覆盖
    ? java.environment_variable：installation 面在视频07提及但未展开 → 建议补充
...
```
> ⚠️ 核心防超时规则：三件产物（SYNTHESIS / EXERCISES / ANKI）必须分三个独立响应生成，
> 对应 Pass 2a / 2b / 2c，每轮 create_file 保存后才启动下一轮。严禁一次响应输出多件产物。
...
Pass 轮次顺序（每轮生成一件产物后立即 create_file 保存，等用户确认后才继续）：
  Pass 1（可选）：outline → chapter_outline.json
                  保存路径：{CHAPTER_DIR}/CHAPTER_SYNTHESIS_{chapter_dir_name}/chapter_outline.json
  Pass 2a：synthesis → CHAPTER_SYNTHESIS_*.md
           知识点 > 20 时：分两步写入（前 10 节 → create_file；剩余节 → replace_string_in_file）
  Pass 2b：exercises → CHAPTER_EXERCISES_*.md（读取磁盘上的 synthesis 文件作为知识源）
  Pass 2c：anki → CHAPTER_ANKI_*.csv + .apkg（读取磁盘上的 synthesis 文件）

注意：C_chapter_synthesis.md 的 Outline Pass 包含强制前置的 Stage 0"内容价值分类门"——
  内容被分为三桶：skill / mental_model / exclude
  SKILL：用户学完后能写代码/执行命令/做技术选型 → 正常展开
  MENTAL_MODEL：直接影响理解和调试的概念模型 → 正常展开
  EXCLUDE：励志/历史故事/行业统计/讲师介绍/课程广告 →
    完全丢弃，不以任何形式出现在任何产物（SYNTHESIS / EXERCISES / ANKI）的任何位置
  分类唯一判据："删掉这段内容后，用户在理解技术原理、写代码、调试或部署时会受影响吗？"
    受影响（含影响技术理解）→ SKILL/MENTAL_MODEL；不受影响 → EXCLUDE
```
...
输出到 {chapter_dir}/CHAPTER_SYNTHESIS_{chapter_dir_name}/：
  ...
  │   ← 末尾包含「📌 本章已引入但后续才完整的概念」预报表（兜底 3，见下）
  │   ← 末尾包含「📝 本章练习索引」表格
...
> **? 兜底机制 3 — 深度完整性预报（CHAPTER_SYNTHESIS 末尾必备）**
>
> 章节综合文档末尾必须包含预报表，让用户知道"现在不懂也没关系"，同时防止知识点遗忘：
>
> ```markdown
> ## 📌 本章已引入但后续才完整的概念
>
...
> |------|---------|------------|------------|
> | GC 垃圾回收 | 1（知道有这东西） | 4（调优参数） | JVM 原理章节 |
> | `public/static/void` | 0.5（见过未学） | 2（运用） | 面向对象章节 |
> | 面向对象三大特征 | 1（名词引介） | 3（多态原理） | OOP 章节 |
> ```
...
按章节逐个完成该查询，拼出全局关联图。
? 不加 chapter_filter 直接 list_all=true 会随课程增长而持续增加 token，仅用于调试/审计目的。
生成 PHASE_MAP.md，包含：
  - 已覆盖概念的关联图（哪些概念互相依赖）
