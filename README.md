...
│   │   ├── COURSE_SUMMARY.md           ← 批量处理汇总（含依赖图+学习路线）
│   │   └── Java基础-视频上/             ← 课程目录
│   │       └── day01-Java入门/          ← 章节目录
...
# 支持格式：.mp4 .mkv .avi .mov .flv .wmv .webm .ts
...
# 5. 使用命令处理视频
# Claude Code 斜杠命令：/batch-process 或 /process-video portable-gpu-worker/videos/01-Java基础.mp4
# Cursor：在对话中直接要求「批量处理视频」或「处理 portable-gpu-worker/videos/01-Java基础.mp4」
```
...
# 关键帧提取
keyframes:
  scene_threshold: 0.35    # 场景切换敏感度 (0-1)
  fallback_interval: 60    # 兜底采样间隔（秒）
...
只要 AI 能读取提示词模板并按规范输出，工作流程完全一致。质量由五项工作原则保障，不依赖特定模型能力。
...
A：项目已包含 `.vscode/mcp.json`，VS Code 1.99+ 内置 MCP 支持，无需额外插件。安装 `GitHub.copilot-chat` 扩展后，在 Copilot Chat 面板切换到 **Agent 模式**，工具栏会出现 MCP 工具图标，可直接调用所有 15 个工具。
