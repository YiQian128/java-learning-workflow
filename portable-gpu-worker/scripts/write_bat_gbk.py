#!/usr/bin/env python3
"""
write_bat_gbk.py - 生成 portable-gpu-worker/0_开始使用.bat

用法:
  python write_bat_gbk.py            # UTF-8 编码（默认，推荐）
  python write_bat_gbk.py --gbk      # GBK 编码（旧版 CMD 兼容模式）

UTF-8 模式：chcp 65001，在现代 Windows CMD 和 IDE 中均正确显示。
GBK 模式：chcp 936，仅在标准 CMD 窗口正确显示，IDE 需手动切换编码。
"""
from pathlib import Path
import sys

USE_GBK = "--gbk" in sys.argv
CHCP_LINE = "chcp 936 >nul 2>&1" if USE_GBK else "chcp 65001 >nul 2>&1"
...
def main():
    ...
    bat_path = root / "0_开始使用.bat"
    content = BAT_TEMPLATE.format(CHCP_LINE)
    enc = "gbk" if USE_GBK else "utf-8"
    bat_path.write_text(content, encoding=enc)
    chcp = "936 (GBK)" if USE_GBK else "65001 (UTF-8)"
    print(f"Written: {bat_path}")
    print(f"Encoding: {enc.upper()}, chcp {chcp}")
    print("Tip: run without --gbk to generate UTF-8 version (default)")
